...
namespace ProEra
{
  public class GameScoreboardUI : MonoBehaviour
  {
    [SerializeField]
    [NotNull]
    private TextMeshProUGUI _quarter;
    [SerializeField]
    [NotNull]
    private TextMeshProUGUI _gameClock;
    [SerializeField]
    private GameScoreboardUI.TeamSection _awayTeam;
    [SerializeField]
    private GameScoreboardUI.TeamSection _homeTeam;
    [Space(10f)]
    [SerializeField]
    ...
    private void SetYardLineHandler()
    {
      ...
      else
      {
        ...
        if ((double) lineByFieldLocation < 1.0)
          (ProEra.Game.MatchState.IsHomeTeamOnOffense ? (TMP_Text) this._homeTeam._yardLine : (TMP_Text) this._awayTeam._yardLine).text = "IN";
        else
          (ProEra.Game.MatchState.IsHomeTeamOnOffense ? (TMP_Text) this._homeTeam._yardLine : (TMP_Text) this._awayTeam._yardLine).text = lineByFieldLocation.ToString();
    ...
    {
      if (newState.Equals((object) EMatchState.End) && AppState.GameMode != EGameMode.k2MD && AppState.GameMode != EGameMode.kAISimGameMode)
        this._linksHandler.Clear();
      try
      {
    ...
    private void SetCompScore(int value) => ((bool) Globals.UserIsHome ? (TMP_Text) this._awayTeam._score : (TMP_Text) this._homeTeam._score).text = value.ToString();

    private void SetUserScore(int value)
    {
      Debug.Log((object) ("SetUserScore: value[" + value.ToString() + "]"));
      ((bool) Globals.UserIsHome ? (TMP_Text) this._homeTeam._score : (TMP_Text) this._awayTeam._score).text = value.ToString();
    }
    ...
    private class TeamSection
    {
      ...
      [NotNull]
      public TextMeshProUGUI _playClock;
      [NotNull]
      public List<Image> timeouts;
      [NotNull]
      public TextMeshProUGUI _yardLine;
      [NotNull]
      public TextMeshProUGUI _downAndDistance;
      [NotNull]
