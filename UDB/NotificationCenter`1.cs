...
namespace UDB
{
  public static class NotificationCenter<T>
  {
    public static void AddListener(string notificationID, Callback<T> handler)
    {
    ...
    public static void RemoveListener(string notificationID, Callback<T> handler)
    {
    ...
    public static void Broadcast(string notificationID, T arg1) => NotificationCenter<T>.Broadcast(notificationID, arg1, NotificationInternal.DEFAULT_MODE);

    public static void Broadcast(string notificationID, T arg1, NotificationMode mode)
    {
      ...
      if (!NotificationInternal.eventTable.TryGetValue(notificationID, out @delegate))
        return;
      if (!(@delegate is Callback<T> callback))
